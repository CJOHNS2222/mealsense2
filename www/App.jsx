...
const SettingsMenu = React.lazy(() => import('./components/SettingsMenu.jsx'));
...
export default function App() {
  ...
  const recipesTabBtnRef = useRef();
  const scheduleTabBtnRef = useRef();
  const shoppingTabBtnRef = useRef();
  ...
  return (
    <div style={darkStyles}>
      ...
      <main style={{ padding: '5.5rem 0 4.5rem 0', minHeight: 'calc(100vh - 10rem)' }}>
        {/* Pass refs to PantryTab and others as needed for spotlight */}
        {tab === 0 && <PantryTab analyzeBtnRef={analyzeBtnRef} inventoryListRef={inventoryListRef} />}
        {tab === 1 && <RecipeTab />}
        {tab === 2 && <ScheduleTab />}
        {tab === 3 && <ShoppingTab />}
        {showSettings && (
          <React.Suspense fallback={<div>Loading settings...</div>}>
            <SettingsMenu
              ...
              onCheckUpdates={() => {
                alert('Check for updates: You are running the latest version.');
              }}
            />
          </React.Suspense>
        )}
        ...
        {showTutorial && (
          <Spotlight
            targetRef={tutorialSteps[tutorialStep].selector}
            message={<>
            ...
            placement={tutorialSteps[tutorialStep].placement}
            onNext={tutorialStep < tutorialSteps.length - 1 ? handleNextTutorial : null}
            onClose={() => {
              setShowTutorial(false);
              window.localStorage.setItem('tutorialComplete', '1');
            }}
      ...
      <nav style={navStyles}>
        {TABS.map((t, i) => (
          <button
            ...
            ref={
              i === 1 ? recipesTabBtnRef :
              i === 2 ? scheduleTabBtnRef :
              i === 3 ? shoppingTabBtnRef :
              undefined
            }
          >
            {t.name}
